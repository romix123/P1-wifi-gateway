...
void addHead(String &str, int refresh) {
  ...
  str += F("<meta charset='utf-8'><meta name=\"viewport\" "
  ...
  str += F("input {width: 100%; box-sizing: border-box; -webkit-box-sizing: "
           "border-box; -moz-box-sizing: border-box; background: #ffffff; "
           "color: #000000;}");
  str += F("input[type=range] {width: 90%;}");
  str += F("select {background: #ffffff; color: #000000;}"); //
  str += F("textarea {resize: vertical; width: 98%; height: 318px; padding: "
           "5px; overflow: auto; background: #ffffff; color: #000000;}");
  str += F("body {text-align: center; font-family: verdana, sans-serif; "
           "background: #ffffff;}");
  str += F("td {padding: 0px;}");
  str += F("button {border: 0; border-radius: 0.3rem; background: #97C1A9; "
           "color: #ffffff; line-height: 2.4rem; font-size: 1.2rem; width: "
           "100%; -webkit-transition-duration: 0.4s; transition-duration: "
           "0.4s; cursor: pointer;}");
...
//  str += F("<meta charset='utf-8'><meta name=\"viewport\"
//  content=\"width=device-width,initial-scale=1,user-scalable=no\"/>"); str +=
//  F("<head><title>Slimme meter</title>"); str += F("<style>"); str += F("div,
//  fieldset, input, select {padding: 5px; font-size: 1em;}"); str +=
//  F("fieldset {background: #ECEAE4;}"); str += F("p {margin: 0.5em 0;}"); str
//  += F("body {text-align: center; font-family: verdana, sans-serif;
//  background: #ffffff;}"); str += F(".p {float: left; text-align: left;}");
//  str += F("</style></head>");
//}
...
void addIntro(String &str) {
  ...
  str += F(
      "<p style='text-align:right;font-size:11px;color:#aaa'><a href='http://");
  str += ipstr;
  str += F(HELP);
}
...
void addUptime(String &str) {
  ...
  if (Telnet) {
    ...
    str += activetelnets;
    str += F(" actieve telnet connecties. Flash size: ");
    str += FlashSize;
    str += F("</div>");
  }
}
...
void handleSetupSave() {
  ...
  if (server.method() == HTTP_POST) {
    ...
    strncpy(config_data.adminPassword, server.arg("adminPassword").c_str(),
            server.arg("adminPassword").length());

    if (server.arg("domo") == "on")
      config_data.domo[0] = 'j';
    else
      config_data.domo[0] = 'n';
    strncpy(config_data.ssid, server.arg("ssid").c_str(),
            server.arg("ssid").length());
    strncpy(config_data.password, server.arg("password").c_str(),
            server.arg("password").length());
    strncpy(config_data.domoticzIP, server.arg("domoticzIP").c_str(),
            server.arg("domoticzIP").length());
    strncpy(config_data.domoticzPort, server.arg("domoticzPort").c_str(),
            server.arg("domoticzPort").length());
    strncpy(config_data.domoticzEnergyIdx,
            server.arg("domoticzEnergyIdx").c_str(),
            server.arg("domoticzEnergyIdx").length());
    strncpy(config_data.domoticzGasIdx, server.arg("domoticzGasIdx").c_str(),
            server.arg("domoticzGasIdx").length());

    if (server.arg("mqtt") == "on")
      config_data.mqtt[0] = 'j';
    else
      config_data.mqtt[0] = 'n';
    strncpy(config_data.mqttIP, server.arg("mqttIP").c_str(),
            server.arg("mqttIP").length());
    strncpy(config_data.mqttPort, server.arg("mqttPort").c_str(),
            server.arg("mqttPort").length());
    strncpy(config_data.mqttUser, server.arg("mqttUser").c_str(),
            server.arg("mqttUser").length());
    strncpy(config_data.mqttPass, server.arg("mqttPass").c_str(),
            server.arg("mqttPass").length());
    strncpy(config_data.mqttTopic, server.arg("mqttTopic").c_str(),
            server.arg("mqttTopic").length());

    strncpy(config_data.interval, server.arg("interval").c_str(),
            server.arg("interval").length());

    if (server.arg("watt") == "on")
      config_data.watt[0] = 'j';
    else
      config_data.watt[0] = 'n';
    if (server.arg("telnet") == "on")
      config_data.telnet[0] = 'j';
    else
      config_data.telnet[0] = 'n';
    if (server.arg("debug") == "on")
      config_data.debug[0] = 'j';
    else
      config_data.debug[0] = 'n';

    config_data.dataSet[0] = 'j';
    config_data
        .dataSet[1] = config_data
                          .adminPassword[server.arg("adminPassword").length()] =
        config_data.ssid[server.arg("ssid")
                             .length()] = config_data
            ...
            config_data.domoticzIP[server.arg("domoticzIP").length()] =
                config_data.domoticzPort[server.arg("domoticzPort").length()] =
                    config_data
                        .domoticzGasIdx[server.arg("domoticzGasIdx").length()] =
                        config_data.domoticzEnergyIdx
                            [server.arg("domoticzEnergyIdx").length()] =
                            config_data
                                .interval[server.arg("interval").length()] =
                                config_data
                                    .mqttIP[server.arg("mqttIP").length()] =
                                    config_data.mqttPort[server.arg("mqttPort")
                                                             .length()] =
                                        config_data.mqttUser
                                            [server.arg("mqttUser").length()] =
                                            config_data
                                                .mqttPass[server.arg("mqttPass")
                                                              .length()] =
                                                config_data.mqttTopic
...
void handleLogin() {
  ...
  str += F("<input type='text' class='form-control' name='adminPassword' "
           "value='' </p>");
  str += F("</fieldset>");
...
void handleUploadForm() {
  ...
  str += F(
      "<form method='POST' action='/update' enctype='multipart/form-data'><p>");
  str += F("<b>Firmware</b><input type='file' accept='.bin,.bin.gz' "
           "name='firmware'></p>");
...
void errorLogin(String returnpage) {
  ...
  str += F("<form action='/");
  str += returnpage;
  str += F("' method='POST'><button class='button "
           "bhome'>Opnieuw</button></form></p>");
  addFoot(str);
  server.send(200, "text/html", str);
  bootSetup = false;
}
...
void setupSaved(String &str) {
  //  str += F("<script>var countDownDate = new Date().getTime()+5000;var x =
  //  setInterval(function() {var now = new Date().getTime();var distance =
  //  countDownDate - now;var seconds = Math.floor((distance % (1000 * 15)) /
  //  1000);document.getElementById(\"timer\").innerHTML = seconds + \" seconden
  //  tot module weer bereikbaar.\";if (seconds < 2)
  //  {location.replace(\"http://p1wifi.local\")}}, 1000);</script>");
  str += F("<fieldset>");
  str += F(SETUP_SAVED_TITLE);
  str += F(SETUP_SAVED_TEXT);
  str += F("</fieldset></p>");
  str += F("<div style='text-align:right;font-size:11px;'><hr/><a "
           "href='http://esp8266thingies.nl' target='_blank' "
           "style='color:#aaa;'>esp8266thingies.nl</a></div></div></fieldset></"
           "body></html>");
}
...
void handleHelp() {
  ...
  str += F("<div style='text-align:right;font-size:11px;'><hr/><form><input "
           "type='button' value='Go back!' onclick='history.back()'></form><a "
           "href='http://esp8266thingies.nl' target='_blank' "
           "style='color:#aaa;'>esp8266thingies.nl</a></div></div></fieldset></"
           "body></html>");
  server.send(200, "text/html", str);
...
void addFoot(String &str) {
  ...
  str += F(FIRMWARE_REV);
  str += version;
  str += F("<br><a href='http://esp8266thingies.nl' target='_blank' "
           "style='color:#aaa;'>esp8266thingies.nl</a>");
