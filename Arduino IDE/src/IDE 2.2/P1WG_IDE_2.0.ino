...
const char* host = "P1test";
#define BLED LED_BUILTIN
#define debug(x) Serial.print(x);if(telnetDebugConnected)telnetD(String(x))
#define debugf(x) Serial.printf(x)
#define debugff(x,y) Serial.printf(x,y)
#define debugfff(x,y,z) Serial.printf(x,y,z)
#define debugln(x) Serial.println(x);if(telnetDebugConnected)telnetDLn(String(x))
#elif DEBUG == 3
...
void setup() {
 ...
 EEPROM.begin(sizeof(struct settings) );
  ...
  if (config_data.dataSet[0] !='j') {
      config_data = (settings) {"n", "orbi","Moesmate","-","8080","1234","1235","sensors/power/p1meter","192.168.1.123","1883", "mqtt", "VerySecret", "30", "j","j","n","n","n","adminpwd"};
  }
...
Telnet = true;
 ...
 debugln("Trying to connect to your wifi network:");
  ...
  if ( rtcValid ) {
    ...
    WiFi.begin( config_data.ssid, config_data.password, rtcData.channel, rtcData.ap_mac, true );
